...
_H2H_LINE = re.compile(
    r"^\s*(\d{1,2})(?:st|nd|rd|th)\s*-\s*([A-Z][A-Z'\- ]+?)\s+"
    r"([\d.]+)L\s+([\d.]+)kg(?:\s*\(([+-][\d.]+)\))?", re.I)
_H2H_MEET = re.compile(
    r"^\s*([A-Z][A-Za-z' ]*?(?:\s+AW)?)\s*-\s*(\d{1,2}/\w{3}/\d{4})\s*$")


def _parse_h2h(block):
    """Meetings where this horse has already met one of today's rivals.

    Direct evidence is worth 6 of the framework's 100 points, and this section is
    the only place on the page carrying it. Each meeting is two lines - the rival
    and this horse - giving finishing position, beaten margin and weight, with
    the weight swing in brackets where there was one.

    Meetings are kept whole rather than flattened into a list of results,
    because a comparison only means something as a pair: "beat CONNOISSEUR" is
    empty without knowing both ran in the same race.
    """
    out = []
    lines = block.splitlines()
    start = None
    for i, ln in enumerate(lines):
        if ln.strip().lower().startswith("head to head"):
            start = i + 1
            break
    if start is None:
        return out
    ...
    for ln in lines[start:]:
        ...
        if not t:
            continue
        mm = _H2H_MEET.match(t)
        if mm:
            if meet and len(meet["runners"]) >= 2:
                out.append(meet)
            meet = {"track": mm.group(1).strip(), "date": mm.group(2),
                    "runners": []}
            continue
        rm = _H2H_LINE.match(t)
        if rm and meet is not None:
            meet["runners"].append({
                "finish": int(rm.group(1)),
                "horse": rm.group(2).strip().upper(),
                "margin": _f(rm.group(3)),
                "weight": _f(rm.group(4)),
                "wt_swing": _f(rm.group(5)) if rm.group(5) else 0.0,
            })
            continue
        # Anything else ends the section - the H2H block is contiguous.
        break
    if meet and len(meet["runners"]) >= 2:
        out.append(meet)
    return out
...
def _parse_recent_runs(block: str) -> list[dict[str, Any]]:
    ...
    for i, ln in enumerate(lines):
        ...
        if run["days_ago"] is None:
            # Older runs are written "2m 7d ago" or "4m 13d ago" rather than in
            # plain days. Leaving these as None silently drops the recency of
            # every run beyond about a month, which is most of a horse's form.
            mo = re.search(r"\((?:(\d+)y\s*)?(?:(\d+)m\s*)?(?:(\d+)d\s*)?ago\)", res, re.I)
            if mo and any(mo.groups()):
                y, mth, d = (_i(g) if g else 0 for g in mo.groups())
                run["days_ago"] = y * 365 + mth * 30 + d

        # The official rating for that run sits as a bare number on the line
        # directly above the literal "OHR", between the "N of M" header and the
        # Results line. It is the single most valuable ability signal on the
        # page, so it is read positionally against its own label rather than by
        # counting lines.
        run["ohr"] = None
        run["prep_run"] = None
        for j in range(i + 1, min(i + 8, len(lines))):
            if lines[j].strip() == "OHR" and j > 0:
                v = lines[j - 1].strip()
        ...
        for j in range(i + 1, min(i + 5, len(lines))):
            pm = re.fullmatch(r"\s*(\d+)U\s*-\s*(\d+)d\s*", lines[j])
            if pm:
                run["prep_run"] = int(pm.group(1))
                break
        ...
        # Two layouts in the wild: Australian pages write "Place on settling",
        # South African ones write "Place at 800m". Both mean the early position,
        # which is what the running style is derived from - so accept either.
        run["settle_pos"] = grab(r"(\d+)(?:st|nd|rd|th) Place on settling", _i)
        if run["settle_pos"] is None:
            run["settle_pos"] = grab(r"(\d+)(?:st|nd|rd|th) Place at 800m", _i)
...
def parse(raw: str) -> tuple[dict[str, Any], list[dict[str, Any]], list[str]]:
    ...
    for r in runners:
        ...
        # Prefer an OHR declared for today; otherwise carry the most recent one
        # from the form lines. Without this the whole Ability category - the
        # second-heaviest in the framework - reads as missing on pages that only
        # show ratings per past run, which is most of them.
        if not _f(r.get("ohr"), 0):
            hist = [x.get("ohr") for x in (r.get("recent_runs") or [])
                    if x.get("ohr")]
            if hist:
                r["ohr"] = hist[0]
                r["ohr_source"] = "last run"
        r.setdefault("ohr", 0)
        r.setdefault("ohr_source", "declared" if _f(r.get("ohr"), 0) else "none")
